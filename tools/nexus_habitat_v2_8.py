...
import nexus_voice_handler as voice_runtime_v27

CONTRACT = "JANUS_NEXUS_HABITAT_V2_8"
ENVELOPE_SCHEMA = "janus.demihead.nexus_envelope.v2_8"
ROUTE_RECEIPT_SCHEMA = "janus.demihead.nexus_route_receipt.v2_8"
...
HEADS.update(
    {
        ...
        "VOICE_STATE_RENDERER": v1.HeadRule(
            role="THE_VOICE_OF_JANUS_ORIGIN_PRIME_PYRAMID_RENDERER",
...
NEW_ROUTES = frozenset(
    {
        ...
        ("VOICE_BROKER", "VOICE_STATE_RENDERER", COSMOS_ORIGIN_PRIME_VOICE_ENVELOPE),
        ("VOICE_STATE_RENDERER", "ECHO_PYRAMID", PYRAMID_PCM_HANDOFF),
        ("VOICE_STATE_RENDERER", "VOICE_BROKER", PYRAMID_AUDIO_RECEIPT),
    }
)
...
def validate_envelope(envelope: Mapping[str, Any]) -> None:
    if not isinstance(envelope, Mapping):
        raise ValueError("Nexus v2.8 envelope must be an object")
    if envelope.get("schema") != ENVELOPE_SCHEMA or envelope.get("contract") != CONTRACT:
        raise ValueError("Nexus v2.8 schema or contract mismatch")
    ...
    for key, expected in required.items():
        if control.get(key) != expected:
            raise ValueError(f"Nexus v2.8 control violation: {key}")
...
def build_cosmos_state_route(packet: Mapping[str, Any], *, envelope_id: str | None = None) -> dict[str, Any]:
    ...
    state_commitment = str(packet["origin_prime"]["state_commitment"])
    return _envelope(
        source="COSMOS",
        target="VOICE_BROKER",
        kind=ORIGIN_PRIME_STATE_PACKET,
        sha256=str(packet["packet_sha256"]),
        locator=f"memory://cosmos-origin-prime/{state_commitment}",
        trace=[],
        envelope_id=envelope_id or f"nexus-v2-8-cosmos-{state_commitment[:16]}",
    )
...
def build_voice_envelope_route(mediation: Mapping[str, Any], *, envelope_id: str | None = None) -> dict[str, Any]:
    ...
    return _envelope(
        source="VOICE_BROKER",
        target="VOICE_STATE_RENDERER",
        kind=COSMOS_ORIGIN_PRIME_VOICE_ENVELOPE,
        sha256=v1.sha256(dict(envelope)),
        locator=f"memory://demihead-cosmos-voice/{mediation['intent_id']}",
        trace=["COSMOS"],
        envelope_id=envelope_id or f"nexus-v2-8-voice-{mediation['intent_id'][:16]}",
    )
...
def validate_voice_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if not isinstance(body, Mapping) or body.get("repository") != cvp.ECHO_REPOSITORY or body.get("revision") != cvp.ECHO_STATE_CHAIN_SHA:
        raise ValueError("PYRAMID_AUDIO_RECEIPT_BODY_SNAPSHOT_INVALID")
...
def build_pcm_handoff_route(receipt: Mapping[str, Any], *, envelope_id: str | None = None) -> dict[str, Any]:
    ...
    return _envelope(
        ...
        envelope_id=envelope_id or f"nexus-v2-8-pcm-{receipt['request']['intent_id'][:16]}",
    )
...
def build_audio_receipt_route(receipt: Mapping[str, Any], *, envelope_id: str | None = None) -> dict[str, Any]:
    ...
    return _envelope(
        ...
        trace=["COSMOS", "VOICE_BROKER"],
        envelope_id=envelope_id or f"nexus-v2-8-receipt-{receipt['request']['intent_id'][:16]}",
    )
...
def habitat_snapshot() -> dict[str, Any]:
    parent_voice = voice_runtime_v27.self_test()
    return {
        "schema": "janus.demihead.habitat_snapshot.v2_8",
        "contract": CONTRACT,
        "parent_contract": v24.CONTRACT,
        "inherited_local_voice_runtime": {
            "contract": voice_runtime_v27.CONTRACT,
            "handler_id": voice_runtime_v27.HANDLER_ID,
            "self_test_status": parent_voice["status"],
            "preserved": True,
        },
        "new_heads": ["VOICE_BROKER", "VOICE_STATE_RENDERER", "ECHO_PYRAMID"],
        "new_routes": [list(route) for route in sorted(NEW_ROUTES)],
        "head_count": len(HEADS),
        "route_count": len(ROUTES),
        "providers": {
            "cosmos": cvp.COSMOS_SHA,
            "voice_profile_authority_snapshot": cvp.VOICE_PROFILE_AUTHORITY_SHA,
            "voice_execution": cvp.VOICE_EXECUTION_SHA,
            "echo_state_chain_tested_snapshot": cvp.ECHO_STATE_CHAIN_SHA,
            "echo_current_descendant": cvp.ECHO_CURRENT_SHA,
        },
        "global_control": {
            "nexus_v2_4_rewritten": False,
            "nexus_v2_7_local_voice_runtime_rewritten": False,
            "legacy_cosmos_proof_gate_rewritten": False,
            "direct_cosmos_to_voice_state_renderer_admitted": False,
